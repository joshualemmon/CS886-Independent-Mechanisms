...
def get_dataset(ds, dl):
	...
	if ds == 'MNIST':
		...
		dataset = tv.datasets.Omniglot(root='./datasets/omniglot/download', background=True, download=dl, transform=transform)
...
def transform_datasets(images, labels, ds, tf):
	...
	tf_data = list(zip(training_transformed, labels_transformed))
	random.shuffle(tf_data)
	training_transformed, labels_transformed = zip(*tf_data)
	if tf == 0:
		training_transformed = transform_paper(training_transformed, labels_transformed)
	elif tf == 1:
		training_transformed = transform_expanded(training_transformed, labels_transformed)
...
def save_dataset(transf, c_imgs, c_lbls, ds, tf):
	...
	tf_type = 'paper' if tf == 0 else 'expanded' 
...
def transform_expanded(imgs, lbls):
	tf_imgs = []
	tf_type = ''
	for i, img in enumerate(imgs):
		if i%11 == 0:
